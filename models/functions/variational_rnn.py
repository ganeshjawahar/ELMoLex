...
def VarFastLSTMCell(input, hidden, w_ih, w_hh, b_ih=None, b_hh=None, noise_in=None, noise_hidden=None):
    ...
    if input.is_cuda:
        ...
        return state(igates, hgates,  hidden[0]) if b_ih is None else state(igates, hgates, hidden[1], b_ih, b_hh)
        ...
        for i in steps:
            if mask is None or mask[i].data.min() > 0.5:
                ...
                if isinstance(hidden, tuple):
                    ...
                    #hx = hx.type(torch.FloatTensor)
                    #cx = cx.type(torch.FloatTensor)
                    #hp1 = hp1.type(torch.FloatTensor)
                    #cp1 = cp1.type(torch.FloatTensor)
...
def StackedRNN(inners, num_layers, lstm=False):
    ...
    def forward(input, hidden, cells, mask):
        ...
        return next_hidden, input, master_output
...
def AutogradVarMaskedRNN(num_layers=1, batch_first=False, bidirectional=False, lstm=False):
    ...
    def forward(input, cells, hidden, mask):
        ...
        nexth, output, master_output = func(input, hidden, cells, mask)

        if batch_first:
            output = output.transpose(0, 1)
        return output, nexth, master_output
