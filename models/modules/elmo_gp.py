...
import sys
import json
import allennlp.modules.elmo as elmo_allen 
#import Elmo as ElmoAllen
...
class ElmoGP(nn.Module):
  def __init__(self, word_dim, num_words, char_dim, num_chars, pos_dim, num_pos, num_xpos, num_filters, kernel_size, hidden_size, num_layers, num_labels,
   ...
   init_emb=False, prelstm_args=None, elmo=False, lattice=None, delex=False, word_dictionary=None, char_dictionary=None, use_gpu=False):
    ...
    if self.elmo_use and not self.DEPRECIATED:
      print(prelstm_args)
      self.prelstm_args = json.load(open(prelstm_args,"r",  encoding="utf8", errors='ignore'))
      dim_enc += 2*self.prelstm_args['hidden_size']
      #dim_enc = 1024
    if lattice:
      if lattice[1]:
    ...
    if self.elmo_use:
      ...
      if prelstm_args.endswith('hdf5'):
        ...
        self.elmo = elmo_allen.Elmo(options_file, prelstm_args, 1, dropout=p_in) # add scalar_mix_parameters=[1,1,1] to do unweighted average of different layers
  ...
  def _get_rnn_output(self, input_word, input_char, input_pos, input_xpos, mask=None, length=None, hx=None, input_morph=None, vocab_expand=None):
    ...
    if self.elmo_use and not self.DEPRECIATED:
      # TODO : Q2 : what to do here : Projection then what ? 
      hid, word = self.elmo(input_word, input_char, input_pos, mask, length, hx)
      hid = torch.stack(hid)
      hid = hid.view(-1, word.size(1), self.prelstm_args['hidden_size']*2, self.prelstm_args['num_layers'])
      elmo_embed = torch.cat([hid, word.unsqueeze(3)], dim=3)
